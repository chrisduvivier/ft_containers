	...
	*******************************************/
	...
	class ConstTreeIterator
	{
		public:
			...
			ConstTreeIterator(Node* node, Node* root) { 
				this->_node_ptr = node;
				this->_root = root;
			}

			ConstTreeIterator(const ConstTreeIterator& ref) : 
				_node_ptr(ref._node_ptr),
				_root(ref._root)
			{}
			...
			// For mutable iterators (non-constant iterators):
			ConstTreeIterator & operator=(const ConstTreeIterator& ref) {
			 	if (*this != ref)
			 		this->_node_ptr = ref._node_ptr;
			 		this->_root = ref._root;
			 	return (*this);
			...
			const_pointer 		operator->() const { return &(_node_ptr->data); }
			...
			friend bool operator== (const ConstTreeIterator& a, const ConstTreeIterator& b) { return (a._node_ptr == b._node_ptr); }
			friend bool operator!= (const ConstTreeIterator& a, const ConstTreeIterator& b) { return (a._node_ptr != b._node_ptr); }
			friend bool operator<= (const ConstTreeIterator& a, const ConstTreeIterator& b) { return (a._node_ptr <= b._node_ptr); }
			friend bool operator>= (const ConstTreeIterator& a, const ConstTreeIterator& b) { return (a._node_ptr >= b._node_ptr); }
			friend bool operator< (const ConstTreeIterator& a, const ConstTreeIterator& b) { return (a._node_ptr < b._node_ptr); }
